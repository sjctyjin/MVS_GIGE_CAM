    ...
    if active_way == "getImagebuffer":
        ...
        while True:
            ...
            if ret == 0:
                ...
                image_array = np.frombuffer(pData, dtype=np.uint8).reshape(stFrameInfo.nHeight, stFrameInfo.nWidth)
                #
                # 根據相機的像素格式轉換圖像 (假設是灰度)
                print(stFrameInfo.enPixelType)
                print("Mono : ",PixelType_Gvsp_Mono8)
                print("RGB : ",PixelType_Gvsp_BayerRG8)
                ...
                if stFrameInfo.enPixelType == PixelType_Gvsp_BayerRG8:
                    image_array = cv2.cvtColor(image_array, cv2.COLOR_BayerRG2RGB)
                print("顯示影像")
